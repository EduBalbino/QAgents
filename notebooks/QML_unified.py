...
log_filename = os.path.join(log_dir, f"QML_unified_{timestamp}.log")
...
df = load_and_sample_csv('data/unified.csv', DEV_RUN_SAMPLE_SIZE)
...
features = ['fwd_iat_mean', 'fwd_iat_std', 'bwd_iat_mean', 'bwd_iat_std', 'duration']
X = df[features]
y = df['is_attack']
